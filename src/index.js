...
const { Thing, Value, Property } = require("webthing");
...
const WebThingsDHT = (opt = config) => {
  ...
  const { id, name, description, properties, interval } = opt;
  ...
  dht.addProperty(
    new Property(dht, "temperature", dht_temperature, properties.temperature)
  );
  dht.addProperty(
    new Property(dht, "humidity", dht_humidity, properties.humidity)
  );
  setInterval(() => {
    try {
